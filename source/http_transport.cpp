...
// Converts an httplib result to our response, throwing on transport failure.
...
void configure(httplib::Client &client) {
  ...
  // Reuse the TCP + TLS connection across requests (off by default in httplib).
  // This is the whole point: it removes the per-request handshake.
  client.set_keep_alive(true);
}

// Times the request, logs the round-trip latency, and converts the result.
...
// One persistent client per host base. httplib::Client is not safe for
// concurrent use, so a single mutex serializes all requests through the
// transport (there is only one socket per host to share regardless).
