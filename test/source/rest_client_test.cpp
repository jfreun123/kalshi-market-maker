...
constexpr std::string_view kRestFillYes =
    R"({"action":"buy","book_side":"bid","count_fp":"10.00",)"
    R"("created_time":"2026-07-04T00:36:35.015675Z","fee_cost":"0.057400",)"
    R"("fill_id":"7b866830-7097-720e-57eb-4bd69ce41509","is_taker":true,)"
    R"("market_ticker":"KXMLB-WSH","no_price_dollars":"0.0900",)"
    R"("order_id":"f20215be-f223-4f55-b136-060d375e2f54","outcome_side":"yes",)"
    R"("side":"yes","subaccount_number":0,"ticker":"KXMLB-WSH",)"
    R"("trade_id":"7b866830-7097-720e-57eb-4bd69ce41509","ts":1783125395,)"
    R"("yes_price_dollars":"0.9100"})";

constexpr std::string_view kRestFillNo =
    R"({"action":"sell","book_side":"ask","count_fp":"1.77",)"
    R"("created_time":"2026-07-04T00:35:18.005281Z","fee_cost":"0.000000",)"
    R"("fill_id":"1ecf2b02-aa35-7ecb-c8d9-9da32aa32ef5","is_taker":false,)"
    R"("market_ticker":"KXWC-CAPT","no_price_dollars":"0.6600",)"
    R"("order_id":"107149ff-23f4-422d-b45f-6eff8e382595","outcome_side":"no",)"
    R"("side":"no","subaccount_number":0,"ticker":"KXWC-CAPT",)"
    R"("trade_id":"1ecf2b02-aa35-7ecb-c8d9-9da32aa32ef5","ts":1783125318,)"
    R"("yes_price_dollars":"0.3400"})";

} // namespace

TEST_F(RestClientTest, GetFillsParsesYesAndNoSides) {
  constexpr int kNoFillPriceCents = 66;
  constexpr int kYesFillPriceCents = 91;
  constexpr long long kYesFillEpochSeconds = 1'783'125'395LL;
  constexpr double kYesFillFeeCents = 5.74;
  auto transport = std::make_unique<FakeTransport>();
  FakeTransport *const transport_raw = transport.get();
  transport_raw->enqueue({kHttpOk, R"({"fills":[)" + std::string(kRestFillYes) +
                                       "," + std::string(kRestFillNo) +
                                       R"(],"cursor":""})"});
  auto client = make_client(std::move(transport));

  const auto fills = client.get_fills();

  ASSERT_EQ(fills.size(), kTwoResults);
  const kalshi::Fill &yes_fill = fills.at(0);
  EXPECT_EQ(yes_fill.trade_id, "7b866830-7097-720e-57eb-4bd69ce41509");
  EXPECT_EQ(yes_fill.order_id, "f20215be-f223-4f55-b136-060d375e2f54");
  EXPECT_EQ(yes_fill.market_ticker, "KXMLB-WSH");
  EXPECT_EQ(yes_fill.side, kalshi::Side::Yes);
  EXPECT_EQ(yes_fill.price_cents, kYesFillPriceCents);
  EXPECT_EQ(yes_fill.quantity, kalshi::Quantity::from_contracts(10));
  EXPECT_TRUE(yes_fill.is_taker);
  EXPECT_DOUBLE_EQ(yes_fill.fee_cents, kYesFillFeeCents);
  EXPECT_EQ(std::chrono::duration_cast<std::chrono::seconds>(
                yes_fill.timestamp.time_since_epoch())
                .count(),
            kYesFillEpochSeconds);
  const kalshi::Fill &no_fill = fills.at(1);
  EXPECT_EQ(no_fill.side, kalshi::Side::No);
  EXPECT_EQ(no_fill.price_cents, kNoFillPriceCents);
  EXPECT_FALSE(no_fill.is_taker);
}

TEST_F(RestClientTest, GetFillsPassesMinTsAndFollowsCursor) {
  constexpr long long kMinTs = 1'783'125'000LL;
  auto transport = std::make_unique<FakeTransport>();
  FakeTransport *const transport_raw = transport.get();
  transport_raw->enqueue({kHttpOk, R"({"fills":[)" + std::string(kRestFillYes) +
                                       R"(],"cursor":"next-page"})"});
  transport_raw->enqueue({kHttpOk, R"({"fills":[)" + std::string(kRestFillNo) +
                                       R"(],"cursor":""})"});
  auto client = make_client(std::move(transport));

  const auto fills = client.get_fills(kMinTs);

  ASSERT_EQ(fills.size(), kTwoResults);
  ASSERT_EQ(transport_raw->recorded_requests().size(), kTwoResults);
  EXPECT_NE(
...
TEST_F(RestClientTest, GetRecentTradesParsesNewestFirstWithPrices) {
  ...
  transport_raw->enqueue(
      {kHttpOk,
       ...
       R"({"count_fp":"2.00",)"
       R"("created_time":"2026-07-04T19:42:56.131595Z","taker_side":"no",)"
  ...
  EXPECT_GT(trades->front().timestamp, trades->back().timestamp);
  EXPECT_EQ(trades->front().yes_price_cents, 37);
  EXPECT_EQ(trades->back().yes_price_cents, 36);
  EXPECT_EQ(trades->front().trade_id, "t1");
  EXPECT_EQ(trades->front().market_ticker, "KXT");
  EXPECT_EQ(trades->front().quantity, kalshi::Quantity::from_fp_string("1.51"));
  EXPECT_EQ(trades->front().taker_side, kalshi::Side::Yes);
  EXPECT_EQ(trades->back().taker_side, kalshi::Side::No);
  ...
  const auto epoch_seconds = std::chrono::duration_cast<std::chrono::seconds>(
                                 trades->front().timestamp.time_since_epoch())
                                 .count();
  EXPECT_EQ(epoch_seconds, 1'783'197'776LL);
}

TEST_F(RestClientTest, GetRecentTradesPrefersTakerOutcomeSideOverLegacy) {
  auto transport = std::make_unique<FakeTransport>();
  transport->enqueue(
      {kHttpOk,
       R"({"cursor":"","trades":[{"count_fp":"1.00",)"
       R"("created_time":"2026-07-04T20:42:56.131595Z","taker_side":"yes",)"
       R"("taker_outcome_side":"no",)"
       R"("ticker":"KXT","trade_id":"t1","yes_price_dollars":"0.3700"}]})"});
  auto client = make_client(std::move(transport));

  const auto trades = client.get_recent_trades("KXT", 1);
